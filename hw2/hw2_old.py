...
def doHW2():
	if os.path.isdir(PATH) and os.path.isfile(PATH+TERM) and os.path.isfile(PATH+DOC):
		...
		print('terms:'+str(len(termFile))+' docs:'+str(len(docFile)))
		...
		while not query == 'n':
			...
			# q tfidf
			q = {word:(1+math.log10(q[word])*math.log10(len(docFile)/termFile[word][0])) if termFile.has_key(word) else 0 for word in query}
			...
			for doc in appearDoc:
				words = {}
				for word in docFile[doc]:
					if words.has_key(word):
						words[word] += 1
					else:
						words[word] = 1
				d[doc] = ({word:((1 + math.log10(words[word]))*math.log10(len(docFile)/termFile[word][0])) for word in words.keys()})
